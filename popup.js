...
window.addEventListener('DOMContentLoaded', () => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
        ...
        if (tab.url === undefined || tab.url.indexOf('chrome') == 0) {
            buttonCont.innerHTML = '<span style="font-family: lobster, sans-serif">Color Picker</span> can\'t access <i>Chrome pages</i>'
        }
        else if (tab.url.indexOf('file') === 0) {
            buttonCont.innerHTML = '<span style="font-family: lobster, sans-serif">Color Picker</span> can\'t access <i>local pages</i>'
            ...
            button.addEventListener("click", () => {
                if (!window.EyeDropper) {
                    GiveMetheChild("#ad5049", 'Your browser does not support the Color Picker API')
                    return
                }

                chrome.tabs.sendMessage(
                    tabs[0].id,
                    { from: "popup", query: "color_picker_clicked" }
                );
                window.close()
            })
